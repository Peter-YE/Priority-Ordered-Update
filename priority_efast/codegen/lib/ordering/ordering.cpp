  ...
  *time1 = coder::toc();
  ...
  for (j = 0; j < i2; j++) {
    ...
    for (b_k = 0; b_k < 401; b_k++) {
      i = b_k * i2 + j;
      d += a[b_k] * Theta1[(tmp_data[i % i2] + 300 * (i / i2)) - 1];
    }
    b_data[j] = d;
  }
  //  ================ Function: Sigmoid ================
  ...
  for (b_k = 0; b_k < 10; b_k++) {
    d = 0.0;
    for (i = 0; i < 301; i++) {
      d += dv[i] * Theta2[b_k + 10 * i];
    }
    h2[b_k] = 1.0 / (std::exp(-d) + 1.0);
  }
  if (!std::isnan(h2[0])) {
    a__2 = 1;
  } else {
